...
{				
	...
	?>
	<div class="updated BW-anpt-terminal_settings_wrap" >
		<p><strong><?php if(!isset($payment_terminals['CUSTOM_ERROR'])){ _e('Settings saved!' );}else{ anptDisplayCustomErrors($payment_terminals['CUSTOM_ERROR']);} ?></strong></p>
	</div>
	<?php
} 
			
if(isset($_POST['anpt_submit_license']) && $_POST['anpt_submit_license'] == 'yes')
{
	$headers  = "MIME-Version: 1.0\n";
	$headers .= "Content-type: text/html; charset=utf-8\n";
	$headers .= "From: 'authorization' <noreply@".$_SERVER['HTTP_HOST']."> \n";
	$subject = "Authorization[Authorize.net Payment Terminal Wordpress v1.3]";
	$wp_url = get_site_url();
	$message = "License: ".$_POST['anpt_licensekey']."<br /> Url: ".$wp_url. "<br /> Username: ".$_POST['codecanyon_username']."<br />Host: ".$_SERVER['HTTP_HOST']."<br />URI: ".$_SERVER['REQUEST_URI'];
	update_option('anpt_license', $_POST['anpt_licensekey']);
	wp_mail("info@convergine.com", $subject, $message, $headers);
	?>
	<div class="updated BW-anpt-terminal_settings_wrap">
		<p><strong><?php _e('License saved! Thank you for purchasing our script, we hope you will enjoy it!' ); ?></strong></p>
	</div>
	<?php
} 	
...
if(empty($anpt_license))
{
	...
	<div class="wrap">
		...
		<form name="anpt_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			<input type="hidden" name="anpt_submit_license" value="yes">
			<p><?php _e("CodeCanyon License Key: " ); ?><input type="text" name="anpt_licensekey" value="" size="40"></p>
			<p><?php _e("CodeCanyon Username: " ); ?><input type="text" name="codecanyon_username" value="" size="40"></p>
			<p class="submit"><input type="submit" name="Submit" value="<?php _e('Save License Key') ?>" /></p>
		</form>   
	</div>
	<?php
}
...
?>
<div class="pt_footer">
	<a href="http://www.convergine.com" target="_blank"><img src="<?php echo $wp_url?>/wp-content/plugins/authorize_net_payment_terminal/images/convergine.png" /></a>
</div>